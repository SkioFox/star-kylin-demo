...
| P03 | ARM64 Ubuntu 基础镜像可启动，但 Intel Mac 的 QEMU Builder 在 QtWebEngine 依赖层多次长时间无完成缓存；2026-07-28 已完整下载并配置依赖，最终在 `libc-bin` post-install trigger 中触发 QEMU `SIGSEGV`。 | Intel 主机上的 ARM 模拟叠加网络波动，且 QEMU 无法稳定执行目标 ARM64 的系统安装脚本。 | Dockerfile 增加镜像源参数、APT 缓存、有限重试和串行下载；最终仍停止 QEMU 出包，不把模拟结果当作 ARM 证据。 | 未关闭；必须改由 ARM Mac、ARM64 Linux/麒麟或远程 ARM64 Builder 原生执行。 |
...
| P14 | 客户实机使用 `star-kylin-demo-offline-amd64-0.1.0-20260726.3` 时，SHA-256 通过但 Kylin APT 2.0 报 `qml-module-qtwebengine`、`libqt5webengine5` 未安装。 | 原安装器把应用和两个本地依赖 DEB 放在同一 `apt-get --no-download install` 事务；该客户 APT 未将前两项本地 DEB 纳入依赖求解候选。 | 保留原归档不改，新增 `20260729.1` 安装器修复版：校验客户基线后，以 `dpkg -i` 固定顺序安装 WebEngine 库、QML 模块、应用。 | 待客户实机用修复版完成安装、启动和卸载回归；禁止执行裸 `apt --fix-broken install`，以免脱离离线依赖集。 |
