...
每个可分发包必须有递增的 Debian release。原生 CMake/CPack 通道使用 `STAR_KYLIN_PACKAGE_RELEASE`，例如 `20260726.3`；产物版本应为 `0.1.0-20260726.3`，不得用同一版本覆盖不同内容的包。
...
验收必须针对已安装到 `/opt/star-kylin-demo` 的包和开始菜单入口进行，不能以构建目录中的可执行文件替代。若麒麟启用 KySec 执行控制，开发 VM 中对临时测试二进制的“始终允许”只用于本地回归；客户机交付应通过客户批准的软件源、包签名或发布者信任链完成一次性安装信任，不能关闭全局执行控制，也不能要求业务用户逐项放行。

当前 thin `.deb` 依赖目标麒麟已具备匹配基线的 Qt/QML/QtWebEngine 运行包。对于无网络客户机，最终交付应是与架构和客户基线绑定的离线包组：应用 `.deb`、已批准的依赖 `.deb`、校验清单和本地安装脚本。不要随应用打包 glibc；QtWebEngine 版本和许可证必须按客户基线及安全策略确认。
