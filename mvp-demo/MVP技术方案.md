...
本方案只服务 `mvp-demo`，目标是在 ARM64 与 x86_64 两类明确的麒麟 V10 验收机上完成以下闭环：
...
用户已补充明确：最终必须得到可在麒麟安装并运行的 ARM64 与 x86_64 两个安装包。因此本方案将双架构 `.deb` 纳入强制交付，覆盖 `MVP-demo需求.md` 早期“不做打包结论”的边界；生产级签名、灰度和在线升级仍不进入 MVP。
...
| 安装交付 | `arm64`、`amd64` 两个 `.deb` + 各自校验值 + 安装运行说明 | 两个包都必须在对应麒麟真机完成安装、启动和卸载闭环 |
...
- MVP DoD 必须同时包含两个包，且两个包都必须在各自麒麟真机通过验收。
...
- ARM64 与 x86_64 两套 Qt/QtWebEngine 开发包及运行依赖的来源说明。
...
进入 T1 双架构构建前，ARM64 与 x86_64 的最小构建及运行依赖、ECharts/Lucide 资源及许可证必须已通过批准的摆渡通道，或已在目标网络的批准源中可获取；归档版本、校验值和可获取证据。仅确认公网来源不视为 T0A/T0B 完成。
...
- `arm64`、`amd64` 两个 `.deb` 分别在干净麒麟验收机完成安装、桌面/命令行启动、完整演示和卸载闭环。
...
| 阶段 | 实施内容 | 退出条件 |
|---|---|---|
| T0A | 分别锁定 ARM64/x86_64 麒麟、CPU、Qt/QtWebEngine、合成器、字体、构建与打包通道 | 两类环境基线表完整；两类真机空 WebEngine 窗口可运行；两套测试 `.deb` 路线可执行 |
| T0B | 确认 Web URL、Origin、原生程序、ECharts 兼容性和 Mock 数据 | Manifest 内容可冻结；许可证可交付 |
| T1 | 创建 `app` CMake、CMake install/CPack 骨架、资源和日志 | Debug/Release 可编译，两类麒麟真机可启动空窗口，ARM64/x86_64 构建通道均可生成测试 `.deb` |
...
| T8 | 双架构 `.deb` 打包、两类干净机回归、基线记录和演示材料 | 两类真机 A01–A13、两个安装包和交付文档齐全 |
...
| 决策 | 当前默认 | 必须确认时间 |
|---|---|---|
| ARM64/x86_64 麒麟 V10 子版本、CPU/型号、X11/Wayland | 两类环境均未知；ARM64 可优先验证，但双架构均为交付门禁 | T1 完成前 |
| Qt/QtWebEngine/Chromium、装载与授权方式 | 优先使用验收镜像兼容版本；系统 Qt 与应用自带 Qt 二选一 | T1 完成前 |
| 安装包架构与构建通道 | `arm64`、`amd64` 两包；两套对应架构 Linux/麒麟构建机；最小离线依赖分别在目标网络就绪 | T1 完成前 |
...
| M6 / A13 安装包 | CMake install、CPack DEB、双架构干净机验收 |
...
- [ ] `mvp-demo/app` 可在 ARM64 与 x86_64 麒麟构建通道分别编译并生成对应 `.deb`。
...
- [ ] 两个 `.deb`、各自 SHA-256、构建安装说明、演示脚本、双架构验收记录和第三方许可证齐全。
