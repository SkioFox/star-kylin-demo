...
app_deb="$bundle_dir/kylin-sky-demo_0.2.1-5_amd64.deb"
...
# Kylin APT may lose an absolute local DEB path after resolving its package name.
# The preflight above pins every dependency, so install the verified local DEBs directly.
dpkg --install "$webengine_deb" "$qml_deb"
dpkg --install "$app_deb"
